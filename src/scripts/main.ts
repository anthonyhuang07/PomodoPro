...
const almCheckmark = document.getElementById("almCheckmark") as HTMLSpanElement;
const fontCheckmark = document.getElementById("fontCheckmark") as HTMLSpanElement;
  ...
  if (storedFontDataUrl) {
    try {
      ...
      persistedFont.load().then(() => {
        (document.fonts as any).add(persistedFont);
...
document.addEventListener('click', function (event) {
  const isClickInsideSettings = settings.contains(event.target as Node);
  const isClickOnSettingsButton = settingsButton.contains(event.target as Node);
...
form.addEventListener('submit', function (e) {
  ...
  if (fontInput.files[0]) {
    ...
    reader.onload = function () {
      ...
      try { localStorage.setItem('customFontDataUrl', fontUrl); } catch { }
...
let pipWin: (Window & { document: Document }) | null = null;
...
async function openPiP() {
  ...
  if (bgImage && bgImage !== 'none') {
    ...
    if (bgSize) d.body.style.backgroundSize = bgSize;
    if (bgPos) d.body.style.backgroundPosition = bgPos;
    if (bgRepeat) d.body.style.backgroundRepeat = bgRepeat;
  } else {
  ...
  h1.id = 'pipCountdown';
  h1.textContent = (countdown as any).textContent || '';
  ...
  try {
    if ((d as any).fonts && (d as any).fonts.ready) {
      (d as any).fonts.ready.then(() => {
...
function updatePiP() {
  ...
  if (el) el.textContent = (countdown as any).textContent || '';
}
